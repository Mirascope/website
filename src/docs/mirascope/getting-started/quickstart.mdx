...
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL")
...
```python
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL", stream=True)
...
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL", response_model=Capital)
...
```python
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL", json_mode=True)
...
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL", json_mode=True, response_model=CityInfo)
def city_info(city: str) -> str:
    return f"Provide information about {city} in JSON format"
...
@llm.call(provider="$PROVIDER_NAME", model="$PROVIDER_MODEL", output_parser=parse_cot)
@prompt_template(
    """
