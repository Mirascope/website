...
{% hint style="warning" %}
**Lilypad self hosting is currently in an open beta.**
...
Interested in participating in the open beta? [Join our community](https://join.slack.com/t/mirascope-community/shared_invite/zt-2ilqhvmki-FB6LWluInUCkkjYD3oSjNA) and send a message to William Bakst. You'll get early access to Pro and Team plan features when self hosting.
{% endhint %}

Lilypad is fully [open-source](open-source.md) and can be self-hosted in your own infrastructure.
...
You should replace this with the URL where you are ultimately hosting Lilypad and making it accessible.
{% endhint %}
...
First, you’ll need to generate and set a JWT secret (e.g. using [https://jwtsecret.com/generate](https://jwtsecret.com/generate)).
...
```bash
LILYPAD_API_BASE_URL=http://localhost:8000
LILYPAD_CLIENT__BASE_URL=http://localhost:8000
```
...
```bash
LILYPAD_API_BASE_URL=http://localhost:8000
LILYPAD_CLIENT_BASE_URL=http://localhost:8000
```

Now you can use Lilypad as [documented](quickstart.md).
