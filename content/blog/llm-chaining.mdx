...
Mirascope works together with [Lilypad](/docs/lilypad), our open source prompt observability platform that allows software developers to version, trace, and optimize language model calls, treating these as non-deterministic functions.
...
To keep track of all this, we developed [Lilypad](/docs/lilypad), an open source prompt engineering framework that automatically versions and traces LLM calls when you simply add the `@lilypad.generation` decorator, ensuring that evaluations are systematic and reproducible:
