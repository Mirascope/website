...
The [strategies](/blog/prompt-engineering-best-practices) below guide LLMs to generate more accurate and context-aware outputs by embedding structures, examples, and external data modules into prompts.
...
Such roles define the structure and expectations of the conversation to a higher degree and are usually [used in conjunction with code](/docs/mirascope/learn/prompts) that dynamically manages the flow of interactions, ensuring consistent responses tailored to user inputs and context.
...
For good reason, the usage of prompt templates is widespread among many advanced prompt engineering libraries and frameworks such as [LangChain](/blog/langchain-prompt-template).

For those applying prompt engineering in code, Mirascope offers an easy way to [bind prompt templates](/docs/mirascope/learn/prompts) to functions and classes.
...
The above prompts show the basics of how prompt chaining works, but for those interested in intuitive ways of chaining LLM calls, Mirascope allows you to use [computed fields and functions](/blog/prompt-chaining).
...
We recommend using a [prompt management system](/blog/prompt-versioning) to track and document changes, compare different versions, and easily revert to previous iterations.
...
An example of a prompt versioning system is [Lilypad](/docs/lilypad), which you can download and install in your local workspace. In Lilypad, every LLM call is automatically versioned and traced for you so you don't have to worry about manually managing your prompts.
...
When building LLM-driven applications, it might be necessary to use [data orchestration](/blog/llm-orchestration) to coordinate flows between the model’s inputs and external systems, and to ensure the desired output is appropriately routed or processed.
...
Our tutorial on [building a web search agent](/docs/mirascope/guides/agents/web_search_agent/) that uses DuckDuckGo to find information relating to your queries provides more details about how automated search works.
...
For detailed information on how RAG works, our blog post on [building LLM pipelines](/blog/llm-pipeline) shows how to set this up using Mirascope’s advanced prompt engineering features.

## 6 Advanced Prompting Techniques

Below we explore six [sophisticated techniques](/blog/prompt-engineering-examples) for improving model reasoning, consistency, and ability to modify its approach based on the prompt or task at hand:
...
You can also see a version of this [RaR prompt in Python](/docs/mirascope/guides/prompt_engineering/text_based/rephrase_and_respond/) in our code tutorials.
...
You can also check out our code tutorial for creating both zero- and few-shot variants of [CoT prompts](/docs/mirascope/guides/prompt_engineering/text_based/chain_of_thought/) using Mirascope.
...
Our [self-ask prompt coding recipe](/docs/mirascope/guides/prompt_engineering/text_based/self_ask/) shows you how to implement both basic and enhanced (with dynamic example selection) versions.

### 4. Tree of Thought (ToT)

A [Tree-of-Thought Prompt](/blog/llm-prompt) allows a language model to explore multiple potential solutions or reasoning paths for a given task, rather than follow a single line of reasoning.
...
See our [coding tutorial](/docs/mirascope/guides/prompt_engineering/chaining_based/chain_of_verification/) for a complete example of building a CoV prompt using Python.
...
Our coding tutorial shows you how to construct both a basic version of the [self-consistency prompt](/docs/mirascope/guides/prompt_engineering/chaining_based/self_consistency/), as well as using automated answer extraction.
...
You can find Mirascope code samples and tutorials both on our [docs](/docs/mirascope) and on our [GitHub page](https://github.com/mirascope/mirascope).
