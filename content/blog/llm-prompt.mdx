...
Mirascope’s `propt_template` decorator (combined with the [call decorator](https://mirascope.com/learn/calls)) serves as a kind of [template](/blog/langchain-prompt-template) to formulate model-agnostic prompts without needing to make major changes to the prompt structure or content.
