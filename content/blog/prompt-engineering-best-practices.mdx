...
These insights are grounded in our own experiences writing prompts since the earliest days of the OpenAI SDK, when we developed our guiding principles in response to our frustrations working with [complex frameworks](/blog/langchain-alternatives) for developing LLM applications.
...
By their very nature, reusable [prompt templates](/blog/langchain-prompt-template) that are designed for one context don’t port easily to others. There are a few reasons for this:
...
The fact there’s little to no reusability with the current generation of LLMs means that modern [prompt engineering ](/blog/prompt-engineering-tools)(still) has little use for automation-even with templates. Case in point: [some libraries](/blog/prompt-flow-vs-langchain) make much of their use of directed acyclic graphs yet the vast majority of prompting work still lies in iteratively optimizing effective prompts by hand until you get the desired result.
...
We find that making the LLM call the [central organizing unit](/blog/engineers-should-handle-prompting-llms) around which everything, including the prompt, gets versioned and tested promotes code cleanliness and maintainability.
...
![Incorrect LLM Function Input Argument Error](/assets/blog/prompt-engineering-best-practices/storylines_incorrect_input_arg.webp)
...
In our experience, prompts become unmanageable past several iterations if we don’t [version track them](/blog/prompt-versioning) as we would any other code. And if you’re rapidly experimenting with, and optimizing prompts, even manual change tracking is better than none at all.

Our own library Lilypad is purpose built for [prompt management](/docs/lilypad), automatically versioning and tracing every LLM call made, including any and all code that could impact the quality of the prompt.
