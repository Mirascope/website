...
**Below, we list five different [prompting tools](/blog/prompt-engineering-tools) that implement prompt management**, starting with an in depth discussion on how we designed [Mirascope](https://github.com/mirascope/mirascope) and Lilypad, our own toolkit and framework, respectively, for developing robust LLM applications, with respect to the discussion above.
...
Our prompt management and versioning framework, [Lilypad](/docs/lilypad), system works along similar lines. Among other things, we:
...
Lilypad offers a [prompt management and versioning](/docs/lilypad) framework that versions and traces every LLM function call automatically. This helps you iterate faster on prompts and their calls rather than worrying about the tedium of versioning and tracing things manually. Ultimately, this provides a more structured way to refine and tests prompts easily while maintaining a clean and organized environment.

The automatically version and trace, all you need to do is add the `lilypad.trace` decorator on top of your LLM function:
...
We include the `if __name__` bit so that you can run `recommend_book` easily during development in isolation. The above example shows using Lilypad with Mirascope, but Lilypad also works with the official provider SDKs (e.g. the previous OpenAI SDK example).
...
def get_available_books() -> list[str]:    # [!code highlight]
    return ["The Name of the Wind"]    # [!code highlight]


@lilypad.trace(versioning="automatic")
@llm.call("openai", "gpt-4o-mini")
def recommend_book(genre: str) -> str:
    books = get_available_books()    # [!code highlight]
...
The first step when evaluating prompts is to look at your data and annotate it. You can use Lilypad to [annotate traces](/docs/lilypad/evaluation/annotations) directly in the UI or through code.
...
[LangSmith](https://www.langchain.com/langsmith) is a tool for managing and optimizing the performance of chains and intelligent agents in LLM applications. Its parent framework is [LangChain](/blog/langchain-alternatives), with which it integrates, although you can use LangSmith on its own.
...
Want to learn more? You can find more Lilypad code samples on both our [documentation site](/docs/lilypad) and [GitHub](https://github.com/Mirascope/lilypad). You can also learn more about [Mirascope](https://www.mirsacope.com), for which Lilypad has first-class support.
