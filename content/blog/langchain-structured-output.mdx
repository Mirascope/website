...
Setting `json_mode=True` in the call decorator will apply [JSON mode](/learn/json_mode/), if it’s supported by your LLM, rendering the outputs as valid JSON: 
...
You can learn more about Mirascope’s response model [here](/learn/response_models/?h=language+model+models+response). 
