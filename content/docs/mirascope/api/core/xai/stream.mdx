---
# AUTO-GENERATED API DOCUMENTATION - DO NOT EDIT
title: mirascope.core.xai.stream
description: API documentation for mirascope.core.xai.stream
---

# mirascope.core.xai.stream

## <ApiType type="Module" module="mirascope.core.xai.stream" path="core/xai/stream" symbolName="stream" /> stream

The `XAIStream` class for convenience around streaming xAI LLM calls.

<Info title="Usage">

[Streams](/docs/mirascope/learn/streams)

</Info>

## Classes

### XAIStream

### <ApiType type="Class" module="mirascope.core.xai.stream" path="core/xai/stream" symbolName="XAIStream" /> XAIStream

A simple wrapper around `OpenAIStream`.

Everything is the same except updates to the `construct_call_response` method and
the `cost` property so that cost is properly calculated using xAI's cost
calculation method. This ensures cost calculation works for non-OpenAI models.

**Bases:** OpenAIStream

<AttributesTable
  attributes={[
  {
    "name": "cost",
    "type_info": {
      "type_str": "float | None",
      "description": null,
      "kind": "union",
      "base_type": {
        "type_str": "Union",
        "description": null,
        "kind": "simple",
        "doc_identifier": "Union"
      },
      "parameters": [
        {
          "type_str": "float",
          "description": null,
          "kind": "simple",
          "doc_identifier": "float"
        },
        {
          "type_str": "None",
          "description": null,
          "kind": "simple",
          "doc_identifier": "None"
        }
      ]
    },
    "description": "Returns the cost of the call."
  }
]}
/>



