...
<Info title="Diagram illustrating the agent flow" collapsible={true} defaultCollapsed={true}>
  ```mermaid
  sequenceDiagram
...
<Info title="Provider-Agnostic Agent" collapsible={true} defaultCollapsed={true}>
<TabbedSection showLogo={true}>
<Tab value="Shorthand">
```python
